---
title: "The influence of non-driving related tasks on take-over quality in highly automated driving - a multivariate meta-analysis"
author: "Tim Certa"
...
```{r function generator, hide=TRUE, message=FALSE}
...
# averaged effect sizes per study (only for comparison with multilevel model)
effects_avg <- 
  effectsizes %>% 
  group_by(study_id) %>%                    # to average all studies per study_id 
  mutate(variance = sum(variance*sample_size)/sum(sample_size), # sample variances are weighted by sample size
         sample_size = mean(sample_size)) %>%                   # sample size is averaged
  distinct(study_id,                        # only 1 effect size per study is kept
           .keep_all = TRUE)
effects_avg <- effects_avg[1:15]
print(effects_avg)

# Effect sizes and variances are weighted by the sample size.
effects_avg_wghtd <- rma.uni(data = effects_avg, 
                         yi = effect_size,
                         vi = variance, 
                         method = "REML",
                         slab = study)
effects_avg_wghtd
confint(effects_avg_wghtd)
```
...
ggplot(data=effectsizes, aes(x=effect_size)) + 
  geom_histogram(breaks=seq(-4, 4, by=0.2),
                 ...
                 fill="black", 
                 alpha = .2) + 
  labs(title="Calculated effect sizes") +
...
plot(cook_ind, type="o", pch=19, xlab="Observed Outcome", ylab="Cook's Distance")
```

Cook's Distances for each Cluster.
```{r}
cook_cluster <- cooks.distance(overall, cluster=effectsizes$study_id)
plot(cook_cluster, type="o", pch=19, xlab="Cluster", ylab="Cook's Distance", xaxt="n")
